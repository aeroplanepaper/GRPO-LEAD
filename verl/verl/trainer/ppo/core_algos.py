...
RESULTS_DIR = "problem_correctness_14b"
...
def logistic_reweight(x, A=0.4, B=1.5, x0=0.75, k=10.0):
    """
    Logistic reweighting function.
    If x >= 0, use the normal logistic function.
    If x < 0, use the reversed version.
    
    Parameters:
      x: correct ratio (scalar or tensor)
      A: minimum weight (default: 0.4)
      B: maximum weight (default: 1.5)
      x0: midpoint (default: 0.75)
      k: steepness (default: 10.0)
    """
    weight = A + (B - A) / (1 + torch.exp(k * (x - x0)))
    return weight
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            if index[i] not in id2input:
                ...
                id2ground_truth[index[i]] = ground_truth[i]['ground_truth']
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
        ...
        for index_i in id2correct_num:
            ...
            data_to_save = {
                "problem_token": id2input[index_i].tolist(),
                "ground_truth": id2ground_truth[index_i],
        ...
        for i in range(bsz):
            # scores[i] = ((scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon))
            scores_i = ((scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon))
            weight = logistic_reweight(id2correct_ratio[index[i]]) if scores_i > 0 else logistic_reweight(1 - id2correct_ratio[index[i]])
            # weight = 1
            scores[i]  = scores_i *  weight
